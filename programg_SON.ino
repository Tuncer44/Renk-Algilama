//Tuncer Yılmaz--Mekatroniğe Giriş-TCS3200 ile renk algılama
...
void loop() {
  ...
  if (K < 70 && M < 70 && Y < 70) //Eğer tüm renklerin yoğunluğu yüksekse: Beyaz
  {
    ...
    lcd.print("Beyaz");
  } else if (K < 100 && Y < M && abs(K - Y) < 30)
  //Kırmızı yoğunluğu yüksek ve yeşille arasındaki değer 30'den azsa: Sarı
  {
    Serial.print("Renk = Sarı   ");
    lcd.print("Sari");
  } else if (K > 300 && M < Y && abs(M - K) < 120)
